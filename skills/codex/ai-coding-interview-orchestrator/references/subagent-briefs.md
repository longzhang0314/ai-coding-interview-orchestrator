...
Use these briefs when the environment supports subAgent execution. If no subAgent tool exists, run the roles sequentially in the main thread.
...
- Subagents should not receive vague “do everything” tasks.
- Give worker agents disjoint file ownership if they are allowed to edit.
- In timed interviews, prefer read-only subagents unless the implementation can be split cleanly.
